  ...
  const getStatusLabel = () => {
    switch (toolCall.name) {
      ...
      case 'retrieve_appointments':
        return toolCall.status === 'in_progress' ? 'Fetching appointments...' : 'Appointments retrieved';
      case 'cancel_appointment':
        return toolCall.status === 'in_progress' ? 'Cancelling appointment...' : 'Appointment cancelled';
      case 'modify_appointment':
        return toolCall.status === 'in_progress' ? 'Modifying appointment...' : 'Appointment modified';
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        toolCall.status === 'completed' && 'border-green-200 bg-green-50 dark:border-green-800 dark:bg-green-950',
        toolCall.status === 'error' && 'border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-950',
        toolCall.status === 'in_progress' && 'border-blue-200 bg-blue-50 dark:border-blue-800 dark:bg-blue-950',
    ...
    >
      ...
      <div className="flex-1">
        ...
        {toolCall.error && (
          <p className="text-xs text-red-600 dark:text-red-400 mt-1">{toolCall.error}</p>
        )}
      </div>
    </motion.div>
  );
...
export function ToolCallDisplay({ toolCalls, className }: ToolCallDisplayProps) {
  ...
  const visibleToolCalls = toolCalls
    .filter((tc) => tc.status === 'in_progress' || 
                   (Date.now() - tc.timestamp < 10000)) // Show for 10 seconds after completion
    .slice(-5);
